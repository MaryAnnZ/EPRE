...
void Renderer::draw(Quad * quad, Framebuffer* framebuffer)
{
	//getShadows (dirLight)
	std::vector<LightNode*> lights = this->getLights(quad);

	this->useShader(quad, lights, framebuffer);

	bindVertexArray(quad->getVao());
...
void Renderer::useShader(MeshNode* node, std::vector<LightNode*> lights, Framebuffer* framebuffer)
{
